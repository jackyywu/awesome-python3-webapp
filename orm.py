...
def log(sql,args=()):
	logging.inof('SQL: %s' % sql)

async def create_pool(loop,**kw):
	logging.info('create detabase connection pool...')
	global __pool
...
async def execute(sql,args,autocommit=True):
	...
	async with __pool.get() as conn:
		if not autocommit:
			await conn.begin()
		try:
			async with conn.curser(aiomysql,DictCursor) as cur:
...
class ModelMetaclass(type):
	...
	def __new__(cls,name,bases,attrs):
		...
		arrts['__primary_key__'] = primaryKey #主键属性名
		arrts['__fields__'] = fields #除主键外的属性名
...
class Model(dict,metaclass=ModelMetaclass):
    ...
    def __getattr__(self,key):
    	try:
			...
			raise ArrtributeError(r"'Model' object has no attribute '%s'" % key)
	...
	async def findAll(cls,where=None,args=None,**kw):
		...
		if limit is not None:
			...
			if isinstance(limit,int):
				sql.append('?')
				args.append(limit)
			elif isinstance(limit,tuple) ans len(limit) == 2:
				sql.append('?,?')
				args.extend(limit)
			else:
